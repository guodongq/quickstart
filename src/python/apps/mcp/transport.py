...
def infer_transport(
        transport: ClientSession | FastMCPServer | AnyUrl | Path | str,
) -> "ClientTransport":
    """
    ...
    """
    # Already a ClientSession? Just return it.
    if isinstance(transport, ClientSession):
        return transport
    ...
    if isinstance(transport, (AnyUrl, str)) and str(transport).startswith("ws"):
        return WebsocketTransport(url=transport)
...
class ClientTransport(abc.ABC):
    ...
    async def client_session(self, **kwargs) -> AsyncIterator[ClientSession]:
        """
        Establish a connection and yields an active, initialized ClientSession.
        ...
        Args:
            **kwargs: Keyword arguments to pass to the ClientSession
    ...
    async def client_session(
        ...
        with catch({McpError: mcperror_handler, Exception: exception_handler}):
            ...
            async with create_connected_server_and_client_session(
                    server=self._fastmcp.mcp_server,
                    **kwargs,
            ) as session:
                yield session
